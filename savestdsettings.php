...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $connect->prepare("SELECT COUNT(*) FROM studentsettings WHERE username = ?");
    ...
    if ($count > 0) {
        ...
        $stmt = $connect->prepare("UPDATE studentsettings SET color = ?, layout = ? WHERE username = ?");
        $stmt->bind_param('sss', $color, $layout, $username);
        $stmt->execute();
    } else {
        // Preferences don't exist, perform an insert
        $stmt = $connect->prepare("INSERT INTO studentsettings (username, color, layout) VALUES (?, ?, ?)");
